...
namespace Exercises.Tests.Classes
{
    ...
    public class FruitTreeTests
    {
        ...
        public void Setup()
        {
            Type type = Type.GetType(typeof(FruitTree).AssemblyQualifiedName);
            Assert.IsFalse(type.IsAbstract, "FruitTree class must not be abstract. Remove the 'abstract' modifier on FruitTree.");
        }
