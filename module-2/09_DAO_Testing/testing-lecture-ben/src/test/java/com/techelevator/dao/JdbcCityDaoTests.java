...
public class JdbcCityDaoTests extends BaseDaoTests {
    ...
    public void getCitiesByState_with_valid_state_returns_correct_cities() {
        ...
        Assert.assertEquals("Different number of cities found than expected", 2, cities.size());
    ...
    private void assertCitiesMatch(City expected, City actual) {
        Assert.assertEquals("City id does not match", expected.getCityId(), actual.getCityId());
        Assert.assertEquals("City name does not match", expected.getCityName(), actual.getCityName());
        Assert.assertEquals("State abbreviation does not match", expected.getStateAbbreviation(), actual.getStateAbbreviation());
        Assert.assertEquals("Population does not match", expected.getPopulation(), actual.getPopulation());
        Assert.assertEquals("Area does not match", expected.getArea(), actual.getArea(), 0.1);
    }
}
