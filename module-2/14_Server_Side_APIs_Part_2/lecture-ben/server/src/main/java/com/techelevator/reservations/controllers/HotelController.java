...
public class HotelController {
    ...
    public void updateReservation(@Valid @RequestBody Reservation reservation, @PathVariable int id) {
        if (reservation.getGuests() == 1 && reservation.getCheckinDate().equals("2024-02-14")) {
            throw new ResponseStatusException(HttpStatus.EXPECTATION_FAILED, "Checking in with no date on Valentine's Day! Ugh!");
        }
        try {
    ...
    public void deleteReservation(@PathVariable int id) {
        if (reservationDao.deleteReservationById(id) < 1) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Reservation not found");
        }
    }
