...
namespace Exercises.Tests.Classes
{
    ...
    public class TelevisionTests
    {
        ...
        public void Setup()
        {
            Type type = Type.GetType(typeof(Television).AssemblyQualifiedName);
            Assert.IsFalse(type.IsAbstract, "Television class must not be abstract. Remove the 'abstract' modifier on Television.");
        }
