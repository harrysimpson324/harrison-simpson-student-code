...
export default {
  ...
  methods: {
    deleteBoard() {
      if (confirm("Are you sure you want to delete this board and all of its associated cards?")) {
        this.isLoading = true;
        boardService.deleteBoard(this.board.id).then(response => {
          ...
          if (error.response) {
            this.$store.commit('SET_NOTIFICATION', "Error deleting board. Response received was '" + error.response.statusText + "'.")
          }
          else if (error.request) {
            this.$store.commit('SET_NOTIFICATION', "Error deleting board. Server could not be reached.");
          }
          else {
            this.$store.commit('SET_NOTIFICATION', "Error deleting board. Request could not be created.");
          }
        })
