...
function hasTeen(num1, num2, num3) {
	const one = (num1 >= 13 && num1 <=19);
	const two = (num2 >= 13 && num2 <=19);
	const three = (num3 >= 13 && num3 <=19);
	if (one || two||three) {
		return true;
	}
	return false;
...
function lastDigit(num1, num2) {
	const oneLast = num1%10;
	const twoLast = num2%10;
	if (oneLast === twoLast) {
		return true;
	}
	return false;
...
function seeColor(str) {
	...
	const doesStartRed = (str.substring(0, 3) === 'red')
	if (doesStartRed) {
		return 'red';
	}
	if (str.lenth < 4) {
...
function cigarParty(numCigars, isWeekend) {
	const inRange = (numCigars >= 40 && numCigars <= 60);
	if (inRange || (numCigars >=40&&isWeekend)) {
		return true;
	}
	return false;
}
...
function filterBigNumbers(intArr) {
	const returnArray = [];
	if (intArr.length == 0) {
		return returnArray;
	}
...
function filterMultiplesOfX(numArray, divisor) {
...
function createObject() {
	const obj = {firstName: 'Harry', lastName: 'Simpson', age: 28};
	return obj;
}
