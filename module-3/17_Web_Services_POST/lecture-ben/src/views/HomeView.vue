  ...
  methods: {
      ...
      boardService.addBoard(this.newBoard).then(response => {
        if (response.status === 201) {
          ...
          this.$store.commit(
                'SET_NOTIFICATION',
                {
                  message: `Board "${newBoard.title}" was added.`,
                  type: 'success'
                }
              );
          this.retrieveBoards();
    ...
    handleErrorResponse(error, verb) {
      if (error.response) {
        ...
        this.$store.commit('SET_NOTIFICATION', "Error " + verb + " board. Server could not be reached.");
      } else {
        this.$store.commit('SET_NOTIFICATION', "Error " + verb + " board. Request could not be created.");
      }
    },
    resetAddForm() {
